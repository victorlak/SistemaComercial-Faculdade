...
export const CardSelector = ({
  ...
  const renderDropdown = () => (
    <View style={styles.dropdownContainer}>
      {options.map((item) => (
        <TouchableOpacity key={item} style={styles.optionButton} onPress={() => handleSelectOption(item)}>
          <Text style={styles.checkmark}>{currentSelection === item ? '✓' : ' '}</Text>
      ...
      <CardContainer>
        <TouchableOpacity onPress={() => setIsExpanded(!isExpanded)} style={styles.periodSelector}>
          <View style={styles.periodSelectorInner}>
            <Text style={styles.periodText}>{selectorLabel}: {currentSelection}</Text>
